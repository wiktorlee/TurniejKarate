...
def get_clubs_endpoint():
    ...
    try:
        ...
        return jsonify({"error": f"Błąd podczas pobierania klubów: {str(e)}"}), 500
...
def logout():
    try:
        ...
        return jsonify({"error": f"Błąd podczas wylogowania: {str(e)}"}), 500
...
def check_auth():
    ...
    try:
        ...
        return jsonify({"error": f"Błąd podczas sprawdzania autoryzacji: {str(e)}"}), 500
