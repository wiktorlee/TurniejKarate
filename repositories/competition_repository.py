...
class CompetitionRepository:
    """
    Repozytorium dla zawodów - używa widoków SQL z Supabase.
    Wszystkie zapytania SQL IDENTYCZNE jak wcześniej!
    Widoki SQL z Supabase - BEZ ZMIAN!
    """
    
    def get_kata_event_info(self, event_id: int, category_kata_id: int) -> Optional[Tuple]:
        """
        Pobiera informacje o evencie i kategorii Kata z widoku.
        SQL IDENTYCZNE jak wcześniej!
        Widok v_kata_competitors z Supabase - BEZ ZMIAN!
        Zwraca: (event_name, event_start_date, event_end_date, category_name) lub None
        """
    ...
    def get_kata_competitors(self, event_id: int, category_kata_id: int) -> List[Tuple]:
        """
        Pobiera listę zawodników Kata z widoku.
        SQL IDENTYCZNE jak wcześniej!
        Widok v_kata_competitors z Supabase - BEZ ZMIAN!
        Zwraca listę krotek: (athlete_code, first_name, last_name, country_code, club_name, nationality, place, points)
        """
        with get_conn() as conn, conn.cursor() as cur:
            cur.execute(f"""
    ...
    def get_kumite_event_info(self, event_id: int, category_kumite_id: int) -> Optional[Tuple]:
        """
        Pobiera informacje o evencie i kategorii Kumite z widoku.
        SQL IDENTYCZNE jak wcześniej!
        Widok v_kumite_competitors z Supabase - BEZ ZMIAN!
        Zwraca: (event_name, event_start_date, event_end_date, category_name) lub None
        """
        with get_conn() as conn, conn.cursor() as cur:
            cur.execute(f"""
    ...
    def get_kumite_competitors(self, event_id: int, category_kumite_id: int) -> Set[str]:
        """
        Pobiera listę kodów zawodników Kumite z widoku.
        SQL IDENTYCZNE jak wcześniej!
        Widok v_kumite_competitors z Supabase - BEZ ZMIAN!
        """
        with get_conn() as conn, conn.cursor() as cur:
